#!/bin/bash
#SBATCH --job-name=streamingvlm-sft
...
#
# 节点数与梯度累积对照表:
#   1 node  (8 GPUs)  -> gradient_accumulation_steps=64
#   2 nodes (16 GPUs) -> gradient_accumulation_steps=32
#   4 nodes (32 GPUs) -> gradient_accumulation_steps=16
#   8 nodes (64 GPUs) -> gradient_accumulation_steps=8
########################################################

# ============== 用户配置区 ==============
NUM_NODES=${SLURM_NNODES:-4}           # 从SLURM获取或默认4节点
GPUS_PER_NODE=8
MASTER_PORT=${MASTER_PORT:-29500}

# 视频处理参数
export VIDEO_MIN_PIXELS=78400          # 100*28*28, 最小视觉token数=100
export FPS_MAX_FRAMES=480              # 每视频最大帧数 (480/2fps = 4min)
export VIDEO_MAX_PIXELS=19267584       # 24576*28*28, 最大视觉token数≈24k
...
if [ -n "$SLURM_JOB_ID" ]; then
    ...
    # 非SLURM模式: 直接使用torchrun
    # 需要在每个节点上分别运行此脚本
    torchrun \
        --nnodes=${NUM_NODES} \
        --nproc_per_node=${GPUS_PER_NODE} \
        --rdzv_id=streamingvlm_${SLURM_JOB_ID:-$$} \
        --rdzv_backend=c10d \
        --rdzv_endpoint=${MASTER_ADDR}:${MASTER_PORT} \
        train.py \
        --deepspeed ./scripts/zero0.json \
